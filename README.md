...
# Description
`alona` is a Python3-based software pipeline for analysis of single cell RNA sequencing (scRNA-seq) data. `alona` is a command-line tool that performs normalization, quality control, clustering and cell type annotation of single cell RNA-seq data ([1][1]). `alona` integrates many state of the art algorithms in scRNA-seq analysis in a single package. In comparison with other scRNA-seq pipelines, `alona` is not designed as an importable library to be used in existing code. One purpose of `alona` is to facilitate single cell data analysis without requiring typing new code; at the same time, `alona` is written in portable and easily modifiable Python3 code. Running `alona` to analyze scRNA-seq data is simple, fast and requires very little tweaking, as most parameters have sensible defaults. The clustering method used in `alona` is similar to [Seurat](https://github.com/satijalab/seurat); i.e., first computing a shared nearest neighbor network. `alona` uses the [Leiden algorithm](https://github.com/vtraag/leidenalg) (an improved version of the Louvain algorithm) to identify tightly connected communities from the graph.
