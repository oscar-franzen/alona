...
def merge_intervals(intervals):
  ...
  for item in intervals:
    loi.append([ item['annotation_start'], item['annotation_stop'] ])
...
def main():
  ...
  with open('gencode.vM21.annotation.gtf','r') as f:
    for line in f:
      ...
      chromosome,annotation_type,annotation_start,annotation_stop,annotation_strand = foo[0],foo[2],foo[3],foo[4],foo[6]
      
      if (int(annotation_start)-int(annotation_stop)) == 0:
        continue
      ...
      if annotation_type == 'exon':
        ...
        coordinates[gene_id].append({ 'annotation_start' : annotation_start,
  ...
  for gene_id in coordinates:    
    ...
    for item in merged:
      #print("$chr\tGENCODE.v27.custom\texon\t$start\t$stop\t1000\t$strand\t.\tgene_id \"$gene\"; gene_name \"$gene\"; transcript_id \"$gene\"; gene_type \"$gene_types\"; transcript_types \"$ttt\"\n");
      
      # transcript_id is not applicable here, however this parameter seems necessary for the file to be viewable in IGV
      print('%s\tGENCODE.metagenes\texon\t%s\t%s\t1000\t%s\t.\tgene_id "%s"; gene_name "%s"; transcript_id "%s"; transcript_types "%s"' %
        (coordinates[gene_id][0]['chr'],
        item[0],
        item[1],
        coordinates[gene_id][0]['annotation_strand'],
        gene_id,
        gene_id,
        gene_id,
